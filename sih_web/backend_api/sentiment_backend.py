...
async def analyze_csv(file: UploadFile = File(...), api_key: str = Depends(verify_api_key)):
    ...
    if "clean_comment" in df_processed.columns and "Label" in df_processed.columns:
        ...
        # Check if the dictionary is not empty before creating the DataFrame
        if all_counts:
            top_words_df = pd.DataFrame(all_counts).fillna(0).astype(int)
            
            # Filter out columns that are not in the colors dictionary to prevent errors
            valid_cols = [c for c in top_words_df.columns if c in colors]
            
            # Only generate the chart if there are valid columns to plot
            if valid_cols:
                fig, ax = plt.subplots(figsize=(6, 4))
                top_words_df[valid_cols].plot(kind="bar", stacked=True, ax=ax, color=[colors.get(c, 'gray') for c in valid_cols])
